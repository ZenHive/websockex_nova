defmodule WebsockexNova.Integration.DeribitComprehensiveIntegrationTest do
  ...
  setup do
    ...
    {:ok, raw_conn} = ConnectionWrapper.open(
      @host, 
      @port, 
      @ws_path, 
      %{
    ...
    {:ok, client_conn} = ClientDeribit.connect(%{
      host: @host,
      transport: :tls,
      transport_opts: transport_opts
    })
  ...
  describe "raw connection functionality" do
    ...
    test "authenticates with valid credentials", %{raw_conn: conn, credentials: credentials, credentials_available: creds_available} do
      if not creds_available do
  ...
  describe "client API functionality" do
    ...
    test "handles multiple concurrent subscriptions", %{client_conn: conn} do
      ...
      results = Enum.map(@test_channels, fn channel ->
        payload = %{
          "jsonrpc" => "2.0",
