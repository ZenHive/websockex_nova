defmodule WebsockexNova.Integration.ConfigPrecedenceWorkingTest do
  ...
  describe "configuration precedence" do
    test "user options override adapter defaults directly" do
      ...
      # Verify precedence
      assert merged.host == "user.deribit.com"      # User wins
      assert merged.timeout == 25_000               # User wins  
      assert merged.custom_field == "from_user"     # User addition
      assert merged.port == 443                     # Adapter default
      assert merged.path == "/ws/api/v2"           # Adapter default
    ...
    test "client module can inject defaults between adapter and user" do
      ...
      assert config.host == "client.deribit.com"    # Client wins over adapter
      assert config.timeout == 30_000               # User wins over client
      assert config.log_level == :warn              # Client wins over adapter
      assert config.client_specific == true         # Client addition
      assert config.extra_field == "user_data"      # User addition
      assert config.port == 443                     # Adapter default
      assert config.path == "/ws/api/v2"           # Adapter default
    end
    ...
    test "nil values in user options don't override defaults" do
      ...
      assert merged.host == nil                     # Nil overwrites
      assert merged.timeout == 25_000               # User wins
      ...
      assert smart_merged.host != nil               # Adapter default preserved  
      assert smart_merged.timeout == 25_000         # User wins
    end
