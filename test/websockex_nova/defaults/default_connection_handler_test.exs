defmodule WebsockexNova.Defaults.DefaultConnectionHandlerTest do
  ...
  describe "DefaultConnectionHandler.init/1" do
    ...
    test "keeps provided state intact" do
      ...
      assert %ClientConn{} = conn
      assert conn.adapter_state[:user_data] == "test"
      assert conn.adapter_state[:custom_field] == 123
    end
  end
  ...
  describe "DefaultConnectionHandler.handle_connect/2" do
    test "stores connection info in the state" do
      ...
      assert updated_conn.adapter_state.reconnect_attempts == 0
      assert Map.has_key?(updated_conn.adapter_state, :connected_at)
    end
    ...
    test "preserves existing state fields" do
      ...
      conn = %ClientConn{
        adapter_state: %{user_data: "important", settings: %{timeout: 5000}}
      }

      assert {:ok, updated_conn} = DefaultConnectionHandler.handle_connect(conn_info, conn)
      assert updated_conn.connection_info == conn_info
      assert updated_conn.adapter_state[:user_data] == "important"
      assert updated_conn.adapter_state[:settings][:timeout] == 5000
    end
  end
  ...
  describe "DefaultConnectionHandler.handle_disconnect/2" do
    test "handles remote disconnection with reconnect" do
      ...
      conn = %ClientConn{
        adapter_state: %{reconnect_attempts: 0, max_reconnect_attempts: 3}
      }

      assert {:reconnect, new_conn} =
               DefaultConnectionHandler.handle_disconnect(disconnect_reason, conn)

      assert new_conn.adapter_state.reconnect_attempts == 1
      assert new_conn.adapter_state.last_disconnect_reason == disconnect_reason
    end

    test "handles local disconnection without reconnect" do
      disconnect_reason = {:local, 1000, "Closed by client"}
      conn = %ClientConn{adapter_state: %{reconnect_attempts: 0}}
    ...
    test "respects max reconnection attempts" do
      ...
      conn = %ClientConn{
        adapter_state: %{reconnect_attempts: 3, max_reconnect_attempts: 3}
      }

      assert {:ok, new_conn} =
               DefaultConnectionHandler.handle_disconnect(disconnect_reason, conn)

      assert new_conn.adapter_state.reconnect_attempts == 3
      assert new_conn.adapter_state.last_disconnect_reason == disconnect_reason
    end
  ...
  describe "DefaultConnectionHandler.handle_frame/3" do
    ...
    test "handles pong frames" do
      ...
      conn = %ClientConn{
        adapter_state: %{last_ping_sent: System.monotonic_time(:millisecond)}
      }

      assert {:ok, new_conn} =
               DefaultConnectionHandler.handle_frame(frame_type, frame_data, conn)

      assert new_conn.adapter_state.last_pong_received
      refute Map.has_key?(new_conn.adapter_state, :last_ping_sent)
    end
