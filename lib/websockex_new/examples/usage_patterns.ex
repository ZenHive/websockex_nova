defmodule WebsockexNew.Examples.UsagePatterns do
  ...
  alias WebsockexNew.{Client, ClientSupervisor}
  ...
  def direct_connection_example do
    ...
    # Send a message
    Client.send_message(client, Jason.encode!(%{
      jsonrpc: "2.0",
      method: "public/test",
      params: %{}
    }))
    ...
    # Check state
    IO.puts("Connection state: #{Client.get_state(client)}")
  ...
  def client_supervisor_example do
    ...
    {:ok, client} = ClientSupervisor.start_client("wss://test.deribit.com/ws/api/v2",
      heartbeat_config: %{type: :deribit, interval: 30_000},
      retry_count: 10
    )
    
    # Client will automatically restart on crashes
    IO.puts("Supervised client started: #{inspect(client.server_pid)}")
    
    # List all supervised clients
    clients = ClientSupervisor.list_clients()
    IO.puts("Active clients: #{length(clients)}")
