defmodule WebsockexNova.ConnectionRegistry do
  ...
  def register(connection_id, transport_pid) when is_reference(connection_id) and is_pid(transport_pid) do
    ...
    case Registry.register(@registry_name, connection_id, transport_pid) do
      {:ok, _pid} ->
        Logger.debug(
          "[ConnectionRegistry] Registered connection_id #{inspect(connection_id)} with transport_pid #{inspect(transport_pid)}"
        )
      ...
      {:error, reason} ->
        Logger.error(
          "[ConnectionRegistry] Failed to register connection_id #{inspect(connection_id)}: #{inspect(reason)}"
        )
  ...
  """
  def update_transport_pid(connection_id, new_transport_pid)
      when is_reference(connection_id) and is_pid(new_transport_pid) do
    case Registry.update_value(@registry_name, connection_id, fn _old_pid -> new_transport_pid end) do
      :error ->
        Logger.warning(
          "[ConnectionRegistry] Failed to update transport_pid for connection_id #{inspect(connection_id)}: Not found"
        )
      ...
      {_old_pid, _new_pid} ->
        Logger.debug(
          "[ConnectionRegistry] Updated transport_pid for connection_id #{inspect(connection_id)} to #{inspect(new_transport_pid)}"
        )
